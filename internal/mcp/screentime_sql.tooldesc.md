...
Queries macOS Screen Time data (CoreDuet knowledge store) using DuckDB SQL dialect. The database includes comprehensive views for analyzing app usage, web browsing patterns, and productivity metrics.
...
## Core Tables
- `ZOBJECT` - Main events table (timestamps in Core Data format)
  - `ZSTREAMNAME`: Event type ('/app/usage', '/app/webUsage', '/app/inFocus', etc.)
  - `ZVALUESTRING`: App bundle IDs or other identifiers
  - `ZSTARTDATE/ZENDDATE`: Event timestamps (TIMESTAMP type in CoreData format)
  - `ZSOURCE`: Foreign key to ZSOURCE table
- `ZSTRUCTUREDMETADATA` - Detailed metadata for events
...
```sql
-- Today's app summary
SELECT * FROM v_today_summary ORDER BY total_minutes DESC;

-- Focus analysis
SELECT * FROM v_daily_focus_metrics 
...
- The table timestamps are in CoreData format, while the View timstampes are in UTC.
- When doing analysis, consider the user's local timezone.  They may not be in UTC.
