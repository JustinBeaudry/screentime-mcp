...
The default Screen Time CoreDuet tables exist along with pre-built views with UNIX timestamps.
...
- Timestamps in original tables are stored in Core Data format (seconds since 2001-01-01)
- To convert to standard timestamps: ADD INTERVAL '978307200 seconds'
- Example: DATE(ZSTARTDATE + INTERVAL '978307200 seconds')
...
### Core Views
- `v_app_usage` - Main app usage data with corrected timestamps
- `v_web_usage` - Web browsing data with domains and URLs
- `v_notifications` - Notification events by app
- `v_app_focus` - App focus events

### Summary Views
- `v_today_summary` - Today's app usage summary (sessions, minutes, avg duration)
- `v_today_web_summary` - Today's web browsing summary
- `v_daily_app_summary` - Daily summaries for all apps
- `v_daily_focus_metrics` - Focus scores and metrics by day

### Analysis Views
- `v_app_switches` - App switching patterns with switch types
- `v_focus_sessions` - Sessions longer than 5 minutes
- `v_hourly_usage_pattern` - Usage patterns by hour
- `v_app_session_patterns` - Work vs personal app identification

### Convenience Views
- `v_today_usage` - Detailed usage for today
- `v_yesterday_usage` - Detailed usage for yesterday  
- `v_week_usage` - Last 7 days of usage
- `v_month_usage` - Last 30 days of usage
...
# Key SQL Syntax Notes:
- Use EXTRACT(HOUR FROM timestamp) instead of strftime for hour extraction
- Use EXTRACT(EPOCH FROM (end - start)) for duration calculations
- Use DATE(timestamp) for date extraction
- JOIN syntax: JOIN table ON condition (not USING)
- Case statements work well for conditional aggregations
- HAVING clause filters after GROUP BY
- Timestamps are in 1994 format (Core Data offset issue)

## Error Prevention Tips
- **Always use proper DuckDB syntax** - no MySQL/SQLite specific functions
- **Test timestamp comparisons carefully** - use ORDER BY and LIMIT first
- **Handle NULLs explicitly** in duration calculations
- **Use CASE statements** for conditional logic
- **Cast types explicitly** when needed
- **Prefer INTERVAL arithmetic** over manual timestamp math
...
## Complex Analysis Views - Usage Examples

### Find web browsing during VSCode usage:
SELECT domain, COUNT(*) as visits, SUM(web_duration_seconds)/60 as minutes
FROM v_web_usage_with_active_app
WHERE active_app_during_visit = 'com.microsoft.VSCode'
  AND usage_date >= CURRENT_DATE - INTERVAL 7 DAY
GROUP BY domain
ORDER BY visits DESC;

### Analyze transition patterns from VSCode:
SELECT 
    to_app,
    transition_speed,
...
WHERE from_app = 'com.microsoft.VSCode'
  AND transition_date >= CURRENT_DATE - INTERVAL 7 DAY
GROUP BY to_app, transition_speed
...
SELECT 
    ...
    AVG(safari_duration_seconds) as avg_duration,
    SUM(safari_duration_seconds)/60 as total_minutes
FROM v_research_sessions
WHERE to_safari_time >= CURRENT_DATE - INTERVAL 7 DAY
GROUP BY coding_app
ORDER BY research_sessions DESC;

## Notes on Temporal Analysis
- The views handle overlapping sessions and timezone conversions
- NULL active_app indicates web browsing outside any app session
- Transition analysis requires ordered window functions
- Research sessions track round-trip patterns to Safari
- All column names match the base views (e.g., use start_time not web_start_time)
